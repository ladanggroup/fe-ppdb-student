...
const Setting = () => {
  ...
  const { showAdmin, updateAdmin, updatePassword, loading, error } =
    useAdminStore();
  ...
  useEffect(() => {
    const getUser = async () => {
      const data = await showAdmin(user.id);
      if (data) {
        setFormData({
          name: data.name || "",
          email: data.email || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate("/admin/dashboard"); // redirect ke list
    } catch (err) {
...
const handleUpdatePassword = async (e) => {
  ...
  try {
    ...
    handleCloseButtonPassword();
  } catch (err) {
    toast.error(error || "Gagal memperbarui password");
    setErrors(err.response.data.errors);
    console.error(err);
  }
};
